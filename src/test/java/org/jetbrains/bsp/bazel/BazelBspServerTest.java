...
//                client::testTargetCapabilities)
  ...
  private List<BazelBspServerSingleTest> submitTestsForExecution(List<BazelBspServerSingleTest> testsToSubmit) {
  ...
  private boolean executeAllTestsAndReturnTrueIfAllPassed(List<BazelBspServerSingleTest> submittedTests) {
    LOGGER.info("Executing tests...");

    return submittedTests.stream()
        .allMatch(test ->
            test.executeTestWithTimeoutAndReturnTrueIfPassed(BazelBspServerTestData.TEST_EXECUTION_TIMEOUT_IN_MINUTES));
  }
