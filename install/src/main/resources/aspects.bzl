...
def _bsp_target_info_aspect_impl(target, ctx):
    ...
    java_target_info = extract_java_info(target, ctx, output_groups) if not "manual" else None
    scala_toolchain_info = extract_scala_toolchain_info(target, ctx, output_groups) if not "manual" else None
