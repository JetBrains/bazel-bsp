...
public class CliOptionsProvider {
  ...
  private io.vavr.control.Option<Path> getJavaPath(CommandLine cmd) {
    return getOptionValue(cmd, JAVA_PATH_SHORT_OPT).map(Paths::get);
  }

  private io.vavr.control.Option<Path> getBazelPath(CommandLine cmd) {
    return getOptionValue(cmd, BAZEL_PATH_SHORT_OPT).map(Paths::get);
  }
