...
class BazelBinarySpecMapperTest {
    ...
    @Nested
    @DisplayName("fun map(projectView): Result<BazelBinarySpec> tests")
    inner class MapTest {
        ...
        fun `should return failure if it isn't possible to deduct bazel path from PATH and bazel path is null`() {
           // given
           val projectView = ProjectView.Builder(bazelBinary = null).build().getOrThrow()
           ...
           bazelBinarySpecTry.exceptionOrNull()!!::cause shouldBe ProjectViewToExecutionContextEntityMapperException::class
           bazelBinarySpecTry.exceptionOrNull()!!.message shouldBe "Mapping project view into 'bazel path' failed! Could not find bazel on your PATH"
        }
        ...
        fun `should return success with deducted bazel path from PATH if bazel path is null`() {
            ...
            val bazelBinarySpec = bazelBinarySpecTry.getOrThrow()
    ...
    @Nested
    @DisplayName("fun default(): Result<BazelBinarySpec> tests")
